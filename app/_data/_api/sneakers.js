...
export async function getSneakersData() {
  try {
...
/* -------------------------------  Get Sneakers Images --------------------------- */
export async function getSneakersImages(folderName) {
  try {
...
/* ------------------------------- Get Sneakers Data with Images --------------------------- */
export async function getSneakersDataWithImages() {
  try {
    // Step 1: Get all Sneakers data objects
    const dataObjects = await getSneakersData();
    ...
    const dataWithImages = await Promise.all(
      dataObjects.map(async (obj) => {
        ...
        const images = await getSneakersImages(folderName);
    ...
    throw new Error("Can't Get Sneakers Data with Images");
  }
}
