...
/* ------------------------------- Get RunningShoes Data with Images --------------------------- */
export async function getRunningShoesDataWithImages() {
  try {
    // Step 1: Get all RunningShoes data objects
    const dataObjects = await getRunningShoesData();
    ...
    const dataWithImages = await Promise.all(
      dataObjects.map(async (obj) => {
        ...
        const images = await getRunningShoesImages(folderName);
    ...
    throw new Error("Can't Get RunningShoes Data with Images");
  }
}
